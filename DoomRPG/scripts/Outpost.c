...
NamedScript MapSpecial void StimInjector()
{
    ...
    int PrevVials[STIM_MAX];
    
    // if you're already in a menu, terminate
    if (Player.InMenu || Player.InShop || Player.OutpostMenu == OMENU_STIMINJECTOR) return;
    ...
    Player.OutpostMenu = OMENU_STIMINJECTOR;
    
    // Empty the injector and populate the previous vials
    for (int i = 0; i <= STIM_MAX - 1; i++)
    {
        Injector[i] = 0;
        PrevVials[i] = Player.Stim.Vials[i];
    }
    ...
    while (true)
    {
        ...
        // Count compounds in the injector
        for (int i = 0; i <= STIM_MAX - 1; i++)
            Amount += Injector[i];
        ...
        for (int i = 0; i <= STIM_MAX - 1; i++)
        {
            if (Injector[i] > 0)
            {
                ...
                DrawBar(StrParam("Stim%d\n", i + 1), X, Y - YOff + 10.0, Injector[i], true);
                X += Injector[i];
            }
        }
        ...
        X = BaseX;
        SetFont("BIGFONT");
        HudMessage("Injector: %d (\Cf%d Credits\C-)\n", Amount, Cost, HUDMSG_PLAIN, 0, CR_GREEN, X, Y - YOff, 0.05);
        ...
        for (int i = 0; i <= STIM_MAX - 1; i++)
        {
            ...
            if (i == Index)
                ...
                Color = CompoundColors[i];
            
            // Vial Bar
            int Amount = (int)(((fixed)Player.Stim.Vials[i] / (fixed)(Player.Stim.VialMax + 1)) * 100.0);
            if (Amount > 100)
                Amount = 100;
            DrawBar(StrParam("Stim%d\n", i + 1), X, Y, Amount);
            ...
            X += 112.0;
            SetFont("SMALLFONT");
            HudMessage("%s: %d/%d\n", CompoundNames[i], Player.Stim.Vials[i], Player.Stim.VialMax, HUDMSG_PLAIN, 0, Color, X, Y, 0.05);
        ...
        if (Buttons == BT_USE && OldButtons != BT_USE)
        {
            if (Amount == 0)
            {
                PrintError("The injector is empty");
                ActivatorSound("menu/error", 127);
            }
            else if (CheckInventory("DRPGCredits") >= Cost)
            {
                // Save the currently held injector
                int Size = Player.Stim.Size;
                int HeldInjector[STIM_MAX];
                for (int i = 0; i <= STIM_MAX - 1; i++)
                    HeldInjector[i] = Player.Stim.Current[i];
                ...
                // Hack to counteract the toxicity that would be given normally by a stim
                ...
                // Restore the previous injector
                Player.Stim.Size = Size;
                for (int i = 0; i <= STIM_MAX - 1; i++)
                    Player.Stim.Current[i] = HeldInjector[i];
        ...
        if (Buttons == BT_SPEED)
        {
            // Reset vials
            for (int i = 0; i <= STIM_MAX - 1; i++)
                Player.Stim.Vials[i] = PrevVials[i];
        ...
        if (((Buttons == BT_LEFT && OldButtons != BT_LEFT) ||
            ...
            Player.DelayTimer > 35.0 * GetActivatorCVarFixed("drpg_menu_repeat")))
        {
            if (Injector[Index] > 0)
            {
                Injector[Index]--;
                Player.Stim.Vials[Index]++;
                ActivatorSound("menu/move", 127);
            }
        }
        ...
        if (((Buttons == BT_RIGHT && OldButtons != BT_RIGHT) ||
            ...
            Player.DelayTimer > 35.0 * GetActivatorCVarFixed("drpg_menu_repeat")))
        {
            if (Player.Stim.Vials[Index] > 0 && Injector[Index] < 100)
            {
                Injector[Index]++;
                Player.Stim.Vials[Index]--;
                ActivatorSound("menu/move", 127);
...
NamedScript MapSpecial void LevelTransport()
{
    ...
    while (true)
    {
        ...
        if (TeleDest->LevelNum > 0)
            HudMessage("%s, level %d - %s\n", TeleDest->LumpName, TeleDest->LevelNum, MapType, HUDMSG_FADEOUT, MENU_ID + 2, CR_ORANGE, 32.1, 80.1, 0.05, 0.5);
        else
            HudMessage("%s - %s\n", TeleDest->LumpName, MapType, HUDMSG_FADEOUT, MENU_ID + 2, CR_DARKGREEN, 32.1, 80.1, 0.05, 0.5);
        ...
        if (!TeleDest->NeedsRealInfo && !TeleDest->UACBase && !TeleDest->UACArena)
        {
            ...
            case MAPEVENT_DRLA_FEEDINGFRENZY:
                AreaText = StrParam("\C[LightBlue]Det%c%ccted: Anom%clous Lif%c%c%c%c\n\n\n       T     H  E       Y        F     %c    E  D\n\n\n          Y     %c        U      R         F     L     3      %c         H\n", 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94));
                break;
            case MAPEVENT_DRLA_OVERMIND:
                AreaText = StrParam("\C[Red]Er%cor: Unable to read addr%c%c%c%c\n\n\n\CgI SEE THE DARKNESS WITHIN YOUR SOUL.\n", 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94), 33 + Random(0, 94));
                break;
            ...
            SetFont("SMALLFONT");
            HudMessage("Monster level (Approx.): %d - %d\n", MonsterMinLevel, MonsterMaxLevel, HUDMSG_FADEOUT, MENU_ID + 9, CR_WHITE, 32.1, 168.1, 0.05, 0.5);
            HudMessage("%s\n", AreaText, HUDMSG_FADEOUT, MENU_ID + 10, CR_WHITE, 32.1, 184.1, 0.05, 0.5);
        }
        else if (!TeleDest->NeedsRealInfo && TeleDest->UACBase)
        {
            ...
            HudMessage("This is a secure UAC military base.\n", HUDMSG_FADEOUT, MENU_ID + 3, CR_DARKGREEN, 32.1, 96.1, 0.05, 0.5);
            SetFont("BIGFONT");
            HudMessage("Area Status\n", HUDMSG_FADEOUT, MENU_ID + 8, CR_GREEN, 32.1, 152.1, 0.05, 0.5);
            SetFont("SMALLFONT");
        ...
        {
            HudMessage("This is a known UAC battle arena site.\n", HUDMSG_FADEOUT, MENU_ID + 3, CR_LIGHTBLUE, 32.1, 96.1, 0.05, 0.5);
        }
